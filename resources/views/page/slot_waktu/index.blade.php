            ...
            <div class="bg-white dark:bg-gray-800 shadow-xl overflow-hidden sm:rounded-lg">
                ...
                <div class="p-6 flex flex-col md:flex-row gap-6">
                    ...
                    <div class="w-full md:w-1/2 bg-gray-100 dark:bg-gray-700 p-6 rounded-xl shadow-inner">
                        ...
                        <form action="{{ route('slot_waktu.store') }}" method="post">
                            @csrf
                            <div class="mb-5">
                                <label for="waktu_mulai" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                    Waktu Mulai
                                </label>
                                <input name="waktu_mulai" type="time" id="waktu_mulai"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
                                    placeholder="Masukan Waktu Mulai disini...">
                            </div>
                            <div class="mb-5">
                                <label for="waktu_selesai" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                    Waktu Selesai
                                </label>
                                <input name="waktu_selesai" type="time" id="waktu_selesai"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
                                    placeholder="Masukan Waktu Selesai disini...">
                            </div>
                            <button type="submit"
                    ...
                    <div class="w-full md:w-1/2">
                        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                ...
                                <tbody>
                                    @php $no = 1; @endphp
                                    @foreach ($slot_waktu as $key => $p)
                                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                            <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                                {{ $slot_waktu->perPage() * ($slot_waktu->currentPage() - 1) + $key + 1 }}
                                            </th>
                                            <td class="px-6 py-4">
                                                {{ $p->waktu_mulai }}
                                            </td>
                                            ...
                                            <td class="px-6 py-4 space-x-2">
                                                <button type="button" data-id="{{ $p->id }}"
                                                    ...
                                                    data-waktu_mulai="{{ $p->waktu_mulai }}"
                                                    data-waktu_selesai="{{ $p->waktu_selesai }}"
                                                    onclick="editSourceModal(this)"
                                                    class="bg-amber-500 hover:bg-amber-600 px-3 py-1 rounded-md text-xs text-white transition">
                                                    Edit
                                                </button>
                                                <button onclick="return ShiftDelete('{{ $p->id }}','{{ $p->waktu_mulai }}')"
            ...
            <div class="w-full md:w-1/2 bg-white dark:bg-gray-800 rounded-lg shadow-xl mx-5">
                ...
                <form method="POST" id="formSourceModal">
                    ...
                    <div class="flex flex-col p-4 space-y-6">
                        <div>
                            <label for="waktu_mulai_edit" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                Waktu Mulai
                            </label>
                            <input name="waktu_mulai" type="time" id="waktu_mulai_edit"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white"
                                placeholder="Masukan Waktu Mulai disini...">
                        </div>
                        <div>
                            <label for="waktu_selesai_edit" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                Waktu Selesai
                            </label>
                            <input name="waktu_selesai" type="time" id="waktu_selesai_edit"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white"
                                placeholder="Masukan Waktu Selesai disini...">
                        </div>
                    </div>
                    <div class="flex items-center p-4 space-x-2 border-t border-gray-200 dark:border-gray-700 rounded-b">
                        ...
                        <button type="button" data-modal-target="sourceModal" onclick="sourceModalClose(this)"
                            class="bg-red-500 m-2 w-40 h-10 rounded-xl text-white hover:bg-red-600 transition">
                            Batal
                        </button>
                    </div>
                </form>
...
<script>
    const editSourceModal = (button) => {
        ...
        let url = "{{ route('slot_waktu.update', ':id') }}".replace(':id', id);

        document.getElementById('title_source').innerText = `UPDATE SLOT WAKTU ${waktu_mulai}`;
        document.getElementById('waktu_mulai_edit').value = waktu_mulai;
        document.getElementById('waktu_selesai_edit').value = waktu_selesai;
    ...
    const ShiftDelete = async (id, nama) => {
        let tanya = confirm(`Apakah anda yakin untuk menghapus Slot Waktu ${nama} ?`);
        if (tanya) {
            await axios.post(`/slot_waktu/${id}`, {
                    '_method': 'DELETE',
